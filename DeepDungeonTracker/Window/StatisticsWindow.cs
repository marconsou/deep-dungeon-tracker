    ...
    public sealed class StatisticsWindow : WindowEx, IDisposable
    {
        ...
        public override void Draw()
        {
            ...
            if (!noData)
            {
                ...
                float baseX;
                float baseY;
                TimeSpan timeBonusMissTotal = TimeSpan.Zero;
                bool isTimeBonusMissScore = false;

                static void CheckForTimeBonusMissScore(ref TimeSpan timeBonusMissTotal, ref bool isTimeBonusMissScore, TimeSpan floorTime)
                {
                    if (isTimeBonusMissScore)
                    {
                        timeBonusMissTotal = TimeSpan.MinValue;
                        isTimeBonusMissScore = false;
                    }

                    if (timeBonusMissTotal != TimeSpan.MinValue)
                    {
                        timeBonusMissTotal += floorTime;
                        if (timeBonusMissTotal > new TimeSpan(0, 30, 0))
                ...
                if (!isSummary)
                {
                    ...
                    for (var i = 0; i < 9; i++)
                    {
                        ...
                        if (floor.Number > 0)
                        {
                            CheckForTimeBonusMissScore(ref timeBonusMissTotal, ref isTimeBonusMissScore, floor.Time);
                ...
                if (statistics.TimeLastFloor != default || statistics.ScoreLastFloor != 0)
                {
                    ...
                    y = baseY;
                    CheckForTimeBonusMissScore(ref timeBonusMissTotal, ref isTimeBonusMissScore, statistics.TimeLastFloor);
